...
fn test_mov_immediate() {
    ...
    assert_eq!(bytes, [0xB8, 0x01, 0x00, 0x00, 0x00]);

    let bytes = assemble_64_bit("mov rax, 0x1234").unwrap().bytes;
    // Full instruction:
    // 48 b8 34 12 00 00 00 00 00 00
    // First optimization:
    // assert_eq!(bytes,  [0x48, 0xC7, 0xC0, 0x34, 0x12, 0x00, 0x00]);
...
fn test_bitwise() {
    ...
    assert_eq!(bytes, [0x21, 0xD8]);

    // or ecx, 0x10
    let bytes = assemble("or ecx, 0x10").unwrap().bytes;
    assert_eq!(bytes, [0x83, 0xC9, 0x10]);
