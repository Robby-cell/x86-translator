...
fn test_mov_register() {
    let bytes = assemble("mov eax, ebx").unwrap();
    assert_eq!(bytes, vec![0x89, 0xD8]);

    let bytes = assemble("mov rax, rbx").unwrap();
    assert_eq!(bytes, vec![0x48, 0x89, 0xD8]);
}

#[test]
fn test_mov_immediate() {
    let bytes = assemble("mov eax, 1").unwrap();
    assert_eq!(bytes, vec![0xB8, 0x01, 0x00, 0x00, 0x00]);

    let bytes = assemble("mov rax, 0x1234").unwrap();
    assert_eq!(
        bytes,
